...
int solve_shroedinger_sweep_convars(mesh * space, mesh * time, Complex * phi, Complex * V, Complex psi_0 ){
	...
	double s2_t_res = 4.*s_res_sq/time->avg_res;
	equation n_eqs = space->points-2;
	...
	for (int j=1; j <= n_eqs; j++) 
		T[j-1] = (trimatrix_t){ 1., -2.*(1. + s_res_sq*V[j]/space->map[j]) + I*s2_t_res, 1.,
					I*s2_t_res*phi[j] - (phi[j-1] -2.*phi[j] + phi[j+1]) + 2.*s_res_sq * V[j]/space->map[j] * phi[j] + 4.*s_res_sq*V[j]*psi_0 };
	...
	T[0].a = 0.;
	/*- right -*/
	T[n_eqs-1].d -= T[n_eqs-1].c * phi[space->points-1];
	T[n_eqs-1].c = 0.;
